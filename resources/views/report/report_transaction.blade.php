      ...
      <div class="card-body">
        <div class="row">
          ...
          <div class="col-12 mt-4">
           ...
           @if(isset($top_products_week) && count($top_products_week) > 0)
            <div class="mt-5 mb-4">
                <h5 class="font-weight-bold mb-3">📊 Top 3 Barang Terlaris Minggu Ini</h5>
                <div class="row">
                @foreach($top_products_week as $index => $product)
                  <div class="col-md-4 mb-3">
                  <div class="card shadow-sm border-0 rounded-lg" style="background: rgba(255, 255, 255, 0.9); backdrop-filter: blur(6px); min-height: 160px;">
                    <div class="card-body px-3 py-3 d-flex flex-column justify-content-between">
                      <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="text-muted small">#{{ $index + 1 }}</span>
                        <span class="badge rounded-pill text-white px-2 py-1" style="font-size: 0.75rem; background-color: {{ ['#4e73df', '#1cc88a', '#f6c23e'][$index % 3] }};">
                          {{ $product->total_terjual }} Terjual
                        </span>
                      </div>
                      <div class="text-truncate font-weight-bold text-dark" style="font-size: 1rem;">
                        {{ $product->nama_barang }}
                      </div>
                      <div class="mt-2 text-muted small">Barang unggulan minggu ini</div>
                    </div>
                  </div>
                </div>
                @endforeach
            ...
            <div class="mt-5 mb-4">
              <h5 class="font-weight-bold mb-3">Top 3 Barang Terlaris Minggu Ini</h5>
              <div class="alert alert-secondary text-muted">Tidak ada data penjualan untuk minggu ini.</div>
            </div>
            @endif
          </div>
        </div>
...
var myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            @foreach($incomes as $income)
            @php
            $unique_ids = \App\Transaction::selectRaw('MIN(id) as id')
                ->whereDate('created_at', $income)
                ->groupBy('kode_transaksi')
                ->pluck('id');

            $total = \App\Transaction::whereIn('id', $unique_ids)->sum('total');
            @endphp
            "{{ $total }}",
            @endforeach
            @endif
