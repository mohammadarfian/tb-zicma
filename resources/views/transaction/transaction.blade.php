          ...
          <div class="row">
            ...
            <div class="col-12 mt-2">
              <table class="table-payment-3">
                <tr>
                  <td>
                    <div class="input-group">
                      ...
                      <input type="text" class="form-control" id="bayar_display" placeholder="Masukkan nominal bayar">
                      <input type="hidden" name="bayar" id="bayar_actual">
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td class="text-right">
                    <button class="btn btn-bayar" type="submit" id="btnBayar">Bayar</button>
                  </td>
                </tr>
              </table>
            </div>
...
$(document).on('click', '.btn-bayar', function(e){
  ...
  // Cek apakah ada baris pada tabel daftar pesanan
  const jumlahPesanan = $('.table-checkout tr').length;

  if (jumlahPesanan === 0) {
    swal("", "Pesanan Kosong", "error");
    return;
  }

  const total = parseInt($('.nilai-total2-td').val());
  const bayar = parseInt($('#bayar_actual').val());
  ...
  // Validasi diskon jika masih dalam mode edit
  if (!$('.diskon-input').is(':hidden')) {
...
<script>
  const inputDisplay = document.getElementById('bayar_display');
  const inputHidden = document.getElementById('bayar_actual');

  inputDisplay.addEventListener('input', function(e) {
    // Ambil hanya angka
    let angka = this.value.replace(/\D/g, '');

    // Simpan ke input hidden (nilai asli)
