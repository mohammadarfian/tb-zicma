...
class SearchManageController extends Controller
{
    ...
    public function searchPage($word)
    {
    	...
    	foreach ($products as $product) {
    		$product_content .= ' (Kode Barang : ' . $product->kode_barang . ', Kategori : ' . $product->kategori . ', Nama Barang : ' . $product->nama_barang . ', Satuan : ' . $product->satuan . ', Merek : '. $product->merek . ', Stok : ' . $product->stok . ', Harga : Rp. ' . number_format($product->harga,2,',','.') . ', Keterangan : ' . $product->keterangan . ')';
    	}
    	// New Product
    	$new_prpduct_content = 'Daftar Barang | Barang Baru : Kode Barang, Nama Barang, Kategori, Satuan, Merek Barang, Stok Barang, Harga Barang || Import(Upload Excel) -> Langkah - Langkah Import : 1. Siapkan data dengan format Excel (.xls atau .xlsx), 2. Jika sudah sesuai pilih file, 3. Klik simpan, maka data otomatis tersimpan';
    	// Supply
